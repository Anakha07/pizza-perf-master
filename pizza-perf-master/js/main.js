...
var items = document.getElementsByClassName('.mover');// as it's value remains the same--put it outside the function
...
function updatePositions() {
  ...
  var itemsLen= items.length; // calculating the length beforehand for better efficiency
  var top=document.body.scrollTop/1250; // calculating beforehand
  var i=0;
  for (; i < itemsLen; i++) {
    var phase = Math.sin((top) + (i % 5));
    items[i].style.left = items[i].basicLeft + 100 * phase + 'px';// changing the style of each element in the array
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var s = 256;
  var i=0;
  var pizzaAppend=document.getElementById('movingPizzas1'); //to avoid the usage of querySelector inside the loop : to make the append work without a query
  for (; i < 24; i++) {
